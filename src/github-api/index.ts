...
export function getLatestRelease({ owner, repo } = repos.nexcordWeb) {
  const url = `https://api.github.com/repos/${owner}/${repo}/releases/latest`;
  return fetchJson<Release>(url);
}

export function getLatestSha({ owner, repo } = repos.nexcordWeb) {
...
export async function getPlatformDownloadLinks() {
  ...
  const release = await getLatestRelease(repos.nexcordDesktop);
  const mobileRelease = await getLatestRelease(repos.nexcordReactNative);
