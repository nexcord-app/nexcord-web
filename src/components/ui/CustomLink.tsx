...
const POST_LINK_REGEX = /^https?:\/\/nexcord\.com\/p\/(\d+)$/i;
const PROFILE_LINK_REGEX = /^https?:\/\/nexcord\.com\/app\/profile\/(\d+)$/i;
const BOT_INVITE_REGEX = /nexcord\.com\/bot\/(\d+)(?:\?perms=(\d+))?/i;
const MSG_LINKS = /^https?:\/\/nexcord\.com(\/app\/servers\/\d+\/\d+.*)$/i;
...
export function CustomLink(props: CustomLinkProps) {
  ...
  const onLinkClick = (e: MouseEvent) => {
    ...
    const isNexcord = href.includes("nexcord.de");

    if (props.isDangerous && !isNexcord) {
      e.preventDefault();
      createPortal((close) => (
