import { template, createComponent, insert, effect, style, setAttribute, delegateEvents, Dynamic } from 'solid-js/web';
import { createContext, useContext, createSignal, createMemo, batch, createComputed, createEffect, onCleanup, untrack, children, Show, Index } from 'solid-js';
...
const ScrollTargetContext = createContext();
...
const createMeasurementsObserver = props => {
  ...
  const isDirectionHorizontal = createMemo(() => (props.direction || 'vertical') === 'horizontal');
...
const createMainAxisPositions = (measurements, axis, getOverscan) => {
  ...
  let prevPosition = 0;
  const positions = createMemo((prev = []) => {
    if (!measurements.isMeasured) {
      return prev;
    }
...
function VirtualContainer(props) {
  ...
  const crossAxisPositions = createMemo(() => createArray(0, state.crossAxis.totalItemCount)); // When items change, old positions haven't yet changed,
  ...
  const MainAxisItems = itemProps => createComponent(Index, {
    get each() {
  ...
  const virtualElements = children(() => // If there less than 2 cross axis columns
  ...
  createComponent(Show, {
    ...
    get fallback() {
      return createComponent(MainAxisItems, {});
    },
    ...
    get children() {
      return createComponent(Index, {
        ...
        children: crossPos => createComponent(MainAxisItems, {
          get crossPos() {
            return crossPos();
          }
...
delegateEvents(["keydown", "focusin", "focusout"]);

export { ScrollTargetContext, VirtualContainer };
